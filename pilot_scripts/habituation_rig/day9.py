"""
openscope_credit_assignment_passive_training_pilot_order1_day9.py
This is code to generate and run a 40-minute habituation stimulus for day 9
...
GABOR_PARAMS = {
                ...
                'reg_len': [600, 600], # range of durations (sec) for seq of regular sets
                'surp_len': [0, 0], # range of durations (sec) for seq of surprise sets
                'block_len': 600, # duration (sec) of each block (1 per kappa (or ori_std) value, i.e. 2)
...
SQUARE_PARAMS = {
                ...
                ###FOR NO SURPRISE, enter [0, 0] for surp_len and [block_len, block_len] for reg_len
                'reg_len': [299, 299], # range of durations (sec) for reg flow
                'surp_len': [0, 0], # range of durations (sec) for mismatch flow
                'block_len': 299, # duration (sec) of each block (1 per direc/size combo, i.e. 4)
...
if __name__ == "__main__":
    ...
    seed = 9 #CHANGE THIS TO MATCH DAY ID
    ...
    gb_ds = [(600, 1200), (1801, 2401)] #1 x block length for each chunk here
    sq_ds = [(0, 599), (1201,1800)] #2x block length + 1 here
