"""
openscope_credit_assignment_passive_training_pilot_order1_day6.py
This is code to generate and run a 10-minute habituation stimulus for day 6
...
GABOR_PARAMS = {
                ...
                'reg_len': [150, 150], # range of durations (sec) for seq of regular sets
                'surp_len': [0, 0], # range of durations (sec) for seq of surprise sets
                'block_len': 150, # duration (sec) of each block (1 per kappa (or ori_std) value, i.e. 2)
...
SQUARE_PARAMS = {
                ...
                ###FOR NO SURPRISE, enter [0, 0] for surp_len and [block_len, block_len] for reg_len
                'reg_len': [74, 74], # range of durations (sec) for reg flow
                'surp_len': [0, 0], # range of durations (sec) for mismatch flow
                'block_len': 74, # duration (sec) of each block (1 per direc/size combo, i.e. 4)
...
if __name__ == "__main__":
    ...
    gb_ds = [(0, 150), (301,451)] #1 x block length for each chunk here
    sq_ds = [(151, 300), (452,601)] #2x block length + 1 here
