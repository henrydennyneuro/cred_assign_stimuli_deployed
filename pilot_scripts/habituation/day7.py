"""
openscope_credit_assignment_passive_training_pilot_order1_day7.py
This is code to generate and run a 20-minute habituation stimulus for day 7
...
GABOR_PARAMS = {
                ...
                'reg_len': [300, 300], # range of durations (sec) for seq of regular sets
                'surp_len': [0, 0], # range of durations (sec) for seq of surprise sets
                'block_len': 300, # duration (sec) of each block (1 per kappa (or ori_std) value, i.e. 2)
...
SQUARE_PARAMS = {
                ...
                ###FOR NO SURPRISE, enter [0, 0] for surp_len and [block_len, block_len] for reg_len
                'reg_len': [149, 149], # range of durations (sec) for reg flow
                'surp_len': [0, 0], # range of durations (sec) for mismatch flow
                'block_len': 149, # duration (sec) of each block (1 per direc/size combo, i.e. 4)
...
if __name__ == "__main__":
    ...
    seed = 7 #CHANGE THIS TO MATCH DAY ID
    ...
    gb_ds = [(300, 600), (901, 1201)] #1 x block length for each chunk here
    sq_ds = [(0, 299), (601,900)] #2x block length + 1 here
