...
class MovStimW(BaseVisualStim, ColorMixin, ContainerMixin):
    """
    ...
    def __init__(self,
        ...
        super(MovStimW, self).__init__(win, units=units, name=name, autoLog=False)#set autoLog at end of init
    ...
    def _newDotsXY(self, nDots):
        ...
        if self.initScr:
            coords = numpy.random.uniform(-0.5, 0.5, [nDots,2])
            self.initScr = False
            return coords
        
        else:
            # For a rectangular window, the base direction is tan(wid/hei)
            base_dir = numpy.rad2deg(numpy.arctan(win_wid/win_hei))
            ...
            for i, val in enumerate(coords):
                if val[0] > win_wid:
                    ...
                    coords[i] = numpy.array([val[1]/win_wid, new_main/win_hei]) - 0.5 # also scaling and shifting
                else:
                    coords[i] = numpy.array([val[0]/win_wid, val[1]/win_hei]) -0.5 # just scaling and shifting
            
            # rotate based on actual direction
            rot_rad = numpy.deg2rad(self.dir - base_dir)
    ...
    def _update_dotsXY(self):
        ...
        if self.fieldShape in  [None, 'square', 'sqr']:
            ...
            if quad == 0 or 1:
                dead = dead+(numpy.array(self._verticesBase[:,0])>0.5)
            else:
                dead = dead+(numpy.array(self._verticesBase[:,0])<0.5)
            if quad == 0 or 3:
                dead = dead+(numpy.array(self._verticesBase[:,1])>0.5)
            else:
                dead = dead+(numpy.array(self._verticesBase[:,0])<0.5)
                
#            dead = dead+(numpy.array(self._verticesBase[:,0])>0.5)+(numpy.array(self._verticesBase[:,1])>0.5)
